...
export const registerSchema = object({
  username: string({ message: "Username is required" })
    ...
    .regex(/^[a-z_]+$/, "Username must be lowercase and can only contain underscores"),
  email: string({ message: "Email is required" })
    .min(1, "Email is required")
    .email("Invalid email"),
  password: string({ message: "Password is required" })
  ...
  re_password: string({ message: "Password is required" })
    .min(1, "Password is required")
    .min(6, "Password must be more than 6 characters")
    .max(32, "Password must be less than 32 characters")
    .regex(
...
export const urlshortenerSchema = object({
  url: string({ message: "URL is required" })
    .url("Invalid URL")
...
export const domainSchema = object({
  domain: string({ message: "URL is required" })
    .url("Invalid URL")
    .min(1, "URL is required")
    .max(1000, "URL must be less than 1000 characters"),
});
...
export const groupSchema = object({
  ...
  description: string().max(1000, "Description must be less than 1000 characters").optional(),
});

export const supportSchema = object({
  name: string({ message: "Password is required" })
    .min(2, { message: "Name must be at least 2 characters long." })
    .max(50, { message: "Name must not exceed 50 characters." }),
  email: string({ message: "Email is required" })
    .min(1, "Email is required")
    .email("Invalid email"),
  message: string({ message: "Password is required" })
