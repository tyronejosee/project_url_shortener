...
user_schemas: dict = {
    ...
    "partial_update": extend_schema(
        ...
        description="Update some fields of a specific user.",
        request=UserWriteSerializer,
        responses={
            200: OpenApiResponse(UserWriteSerializer, description="OK"),
            400: OpenApiResponse(description="Bad request"),
    ...
    "me": extend_schema(
        ...
        description=(
            ...
            "(Get, update, partial update, delete)."
        ),
        tags=["users"],
    ),
    "activation": extend_schema(
        ...
        description=(
            "Activates a user account and sends a confirmation email if configured."
        ),
        responses={
            204: OpenApiResponse(
    ...
    "resend_activation": extend_schema(
        ...
        description=(
            "Resends an activation email to the user if the user "
            "is not active and activation emails are enabled in the settings."
        ),
        responses={
            204: OpenApiResponse(
    ...
    "set_password": extend_schema(
        ...
        description=(
            "Sets a new password for the user and sends a confirmation "
            "email if configured. The user is logged out if configured, "
            "or the session is updated."
        ),
        responses={
            204: OpenApiResponse(description="No Content"),
            400: OpenApiResponse(description="Bad Request"),
    ...
    "reset_password": extend_schema(
        ...
        description=(
            "Sends a password reset email to the user if the email "
            "address is associated with an active account."
        ),
        responses={
            204: OpenApiResponse(description="No Content"),
            400: OpenApiResponse(description="Bad Request"),
    ...
    "reset_password_confirm": extend_schema(
        ...
        description=(
            ...
            "for the user if the token is valid."
        ),
        responses={
            204: OpenApiResponse(description="No Content"),
            400: OpenApiResponse(description="Bad Request"),
...
token_obtain_pair_schemas: dict = {
    "post": extend_schema(
        summary="Token Obtain Pair",
        description=(
            "Endpoint to obtain a pair of JWT tokens (access and refresh) "
...
token_refresh_schemas: dict = {
    "post": extend_schema(
        summary="Token Refresh",
        description=(
            "Endpoint to refresh an access token by providing a valid refresh token."
        ),
        tags=["tokens"],
    ),
}
...
token_verify_schemas: dict = {
    "post": extend_schema(
        summary="Token Verify",
        description=(
            "Endpoint to verify the validity of a token."
            "This view indicates whether the provided token "
            "is valid but does not provide information on its "
...
provider_auth_schemas: dict = {
    "get": extend_schema(
        ...
        description=(
            "Provides the URL for authorization by interacting "
            "with the specified provider. Validates the `redirect_uri` "
            "against allowed URIs and sets it in the session."
        ),
        tags=["socials"],
    ),
    "post": extend_schema(
        ...
        description=(
            ...
            "and returns the URL for authorization."
        ),
        tags=["socials"],
    ),
}
